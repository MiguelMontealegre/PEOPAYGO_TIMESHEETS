  ...
  implements OnInit
{
  ...
  override ngOnInit(): void {
    ...
    const readUserRoleSale = this.authenticationService.authService.model.roles.find(e => e.name === 'SALE_USER');
    if(readUserRoleSale && this.authenticationService.authService.model.roles.length === 1){
      this.menuItems = this.menuItems.filter(item => {
